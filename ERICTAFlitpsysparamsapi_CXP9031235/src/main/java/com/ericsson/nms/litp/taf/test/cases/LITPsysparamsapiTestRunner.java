...
public class LITPsysparamsapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPsysparamsapiTestRunner.class);
    ...
    @TestId(id = "CXP9031235-1", title = "Upgrade ERIClitpsysparamsapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpsysparamsapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("sysparamsapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031235-2", title = "Run python test cases for ERIClitpsysparamsapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpsysparamsapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpsysparamsapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031235-3", title = "Parse xml outputs from python tests")
